...
class PlanValidator:
    ...
    def validate_plan(destinations: List[PlanDestination]) -> List[str]:
        ...
        if len(attractions) < 2:
            warnings.append("Lịch trình có vẻ hơi ít chỗ chơi, bạn có muốn mình gợi ý thêm vài địa điểm thú vị gần đó không?")

        # 2. Kiểm tra chỗ ăn uống (Quên chọn chỗ ăn)
        has_food = any(d.type == DestinationType.restaurant for d in destinations)
        if not has_food:
            warnings.append("Mình chưa thấy bạn chọn nhà hàng hay quán cafe nào. Đi chơi nhớ phải nạp năng lượng nhé!")
