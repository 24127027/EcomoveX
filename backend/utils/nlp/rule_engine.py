...
class ParseResult:
    def __init__(self, intent: str = Intent.UNKNOWN, entities: Dict[str, Any] = None, confidence: float = 0.0):
        self.intent = intent
        self.entities = entities or {}
        self.confidence = confidence
    
    def __repr__(self):
        return f"ParseResult(intent={self.intent}, entities={self.entities}, confidence={self.confidence})"
...
class RuleEngine:
    ...
    def __init__(self):
        self.add_syn_vi = ["thêm", "thêm vào", "cho thêm", "bổ sung", "đưa thêm", "thêm nữa"]
        self.add_syn_en = ["add", "insert", "put", "include", "append", "create"]
        
        self.remove_syn_vi = ["xoá", "xóa", "bỏ", "loại bỏ", "gỡ bỏ", "xoá đi", "bỏ đi", "hủy"]
        self.remove_syn_en = ["remove", "delete", "cancel", "drop", "eliminate"]
        
        self.modify_syn_vi = ["đổi", "thay", "sửa", "chỉnh", "cập nhật", "thay đổi", "điều chỉnh"]
        self.modify_syn_en = ["change", "modify", "update", "edit", "adjust", "alter"]
        
        self.suggest_syn_vi = ["gợi ý", "đề xuất", "giới thiệu", "tư vấn", "recommend", "thay thế"]
        self.suggest_syn_en = ["suggest", "recommend", "advise", "propose", "alternative"]
        ...
        self.activity_keywords_vi = ["nhà hàng", "khách sạn", "bảo tàng", "công viên", "biển", "núi", "chùa", "đền", "phố cổ", "cafe", "quán"]
        self.activity_keywords_en = ["restaurant", "hotel", "museum", "park", "beach", "mountain", "temple", "cafe", "shop"]
    ...
    def classify(self, text: str) -> ParseResult:
        ...
        if self._check_keywords(text_lower, self.route_syn_vi + self.route_syn_en):
            match = re.search(r"(?:từ|from)\s+(.+?)\s+(?:đến|tới|to)\s+(.+?)(?:\s|$)", text, re.IGNORECASE)
            if match:
                return ParseResult(Intent.GET_ROUTE, {
        ...
        if self._check_keywords(text_lower, self.view_syn_vi + self.view_syn_en):
            if any(word in text_lower for word in ["kế hoạch", "plan", "lịch trình", "itinerary", "hiện tại"]):
