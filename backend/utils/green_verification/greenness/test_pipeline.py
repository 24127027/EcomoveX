...
if __name__ == "__main__":
    ...
    model_check_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), MY_MODEL_NAME)
    if not os.path.exists(model_check_path):
        print(f"❌ CẢNH BÁO: Không tìm thấy file model '{MY_MODEL_NAME}' trong thư mục greenness!")
    # --- SỬA ĐOẠN NÀY ---
    ...
    if not os.path.exists(img_path):
        ...
        if result:
            ...
            cv2.imwrite(os.path.join(current_dir, "output_tree.png"), result["tree_img"])
