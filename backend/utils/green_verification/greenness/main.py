...
async def verify_by_place_id(
    place_id: str, output_dir: Path, threshold: Tuple[float, float], db: AsyncSession
):
    """Verify a destination by place_id."""
    print("Initializing TreeSegmenter...")
    segmentation = TreeSegmenter(model_name="best.pt")

    print("Creating orchestrator...")
    orchestrator = GreenVerificationOrchestrator(
        segmentation=segmentation, green_threshold=threshold, use_depth_weighting=True
    )

    print(f"Processing place: {place_id}")
    result = await orchestrator.verify_destination_by_place_id(
        db=db, place_id=place_id, save_to_db=True
    )
...
async def verify_by_coordinates(
    ...
    result = await orchestrator.verify_destination_by_coordinates(
        db=db, latitude=latitude, longitude=longitude, save_to_db=False
    )
...
async def main():
    parser = argparse.ArgumentParser(description="Green Verification for Destinations")
    parser.add_argument("--place-id", type=str, help="Google Place ID to verify")
    ...
    if not args.place_id and not (args.latitude and args.longitude):
        parser.error(
            "Either --place-id or both --latitude and --longitude must be provided"
        )
