...
class GreenVerificationRequest(BaseModel):
    ...
    image_urls: List[str] = Field(
        ..., min_length=1, max_length=20, description="List of image URLs to verify"
    )
    green_threshold: Optional[float] = Field(
...
class ImageVerificationResult(BaseModel):
    ...
    green_score: float = Field(
        ..., description="Final green score (proportion * depth_weighted)"
    )
