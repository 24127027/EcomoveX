...
from services.llm_service import ChatbotMessageService
...
async def send_message(chat_msg: ChatMessage, db: AsyncSession = Depends(get_db)):
    result = await ChatbotMessageService.process_message(db, chat_msg.user_id, chat_msg.session_id, chat_msg.message)
    return result
