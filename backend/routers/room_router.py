...
@router.get(
    "/direct-rooms",
    response_model=list[RoomDirectResponse],
...
@router.get(
    "/direct-rooms/{direct_room_id}",
    response_model=RoomDirectResponse,
    status_code=status.HTTP_200_OK,
)
...
async def create_room(
    ...
    return await RoomService.create_room(db, current_user["user_id"], room_data)
...
async def remove_users_from_room(
    ...
    return await RoomService.remove_users_from_room(
        db, current_user["user_id"], member_data.ids, room_id
    )
