from sqlalchemy import CheckConstraint, Enum as SQLEnum, Column, DateTime, ForeignKey, Index, Integer, PrimaryKeyConstraint, String, UniqueConstraint
...
class Room(Base):
    __tablename__ = "rooms"
    __table_args__ = (
        Index('ix_room_type_created', 'room_type', 'created_at'),
    )

    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(255), nullable=True)
    avatar_blob_name = Column(String(255), ForeignKey("metadata.blob_name", ondelete="SET NULL"), nullable=True, index=True)
