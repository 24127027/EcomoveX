...
export default function SigninPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen items-center justify-center bg-white">
      ...
      >
        <h1
          className={`${knewave.className} text-6xl text-green-600 mb-4 text-center translate-x-40 -translate-y-40`}
        >
          Ecomove<span className="text-green-500">X</span>
        </h1>

        <p
          className={`${josefin_sans.className} text-green-600 mb-12 text-center text-xl leading-relaxed translate-x-42 -translate-y-40`}
        >
          Your Trip. Your impact. Your choice.
        </p>
        <form
          onSubmit={handleSubmit}
          className="flex flex-col gap-4 w-full max-w-sm -translate-y-40"
        >
          <input
            type="text"
            name="username"
          ...
          <Link
            ...
            className={`${abhaya_libre.className}  w-full text-lg text-green-500 translate-x-90 hover:text-green-800`}
          >
            Forgot Password?
          </Link>
          <button
